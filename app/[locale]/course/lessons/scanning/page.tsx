...
export default function Page() {
  return (
    <LessonShell slug="scanning">
      <L
        ar={<>
          <Section title="الفرق بين الاستطلاع و المسح">
            <Analogy>الاستطلاع = جلوس في المقهى أمام البنك تراقبه. المسح = الدخول إلى الردهة و محاولة فتح كل باب لترى أيها مفتوح. هنا أنت تتفاعل مباشرة مع الهدف.</Analogy>
          </Section>
          <Section title="مسح المنافذ — Port Scanning بـ Nmap">
            <p>الـ port هو «باب» الخدمة. كل خدمة تستمع على رقم منفذ. nmap هو الأداة الأم للمسح.</p>
            <h3>مراحل المسح بالترتيب</h3>
            <Step n={1} title="مسح سريع لمعرفة الأبواب المفتوحة">
              <Terminal lines={[
                { p: "nmap -sS -p- --min-rate=2000 -T4 target.gov -oA quick" },
                { o: "PORT      STATE    SERVICE\n22/tcp    open     ssh\n80/tcp    open     http\n443/tcp   open     https\n3306/tcp  open     mysql\n8080/tcp  open     http-proxy" },
              ]} />
            </Step>
            <Step n={2} title="مسح عميق على المنافذ المفتوحة فقط">
              <Terminal lines={[
            ...
            <Step n={3} title="استخدام سكربتات NSE">
...
nmap -p443 --script ssl-enum-ciphers,ssl-cert target.gov`}</Code>
            ...
            <Callout kind="warn" title="تجنّب الكشف">
              <ul>
                <li>-T2 أبطأ لكن أصعب اكتشافاً.</li>
                <li>-f لتجزئة الحزم.</li>
                <li>--source-port 53 لتمرير بعض الـ firewalls.</li>
                <li>-D RND:10 لتقليد عناوين decoy.</li>
              </ul>
            </Callout>
          </Section>
          <Section title="فحص الثغرات — Vulnerability Scanning">
            <TwoCol>
              <Card title="Nuclei" color="red">مفتوح المصدر، يحتوي على آلاف الـ templates لـ CVEs معروفة.</Card>
              <Card title="Nikto / OpenVAS / Nessus" color="amber">ماسحات تقليدية تشمل آلاف الفحوصات على web و البنية التحتية.</Card>
            </TwoCol>
            <Terminal lines={[
          ...
          <Section title="تعداد المسارات و الملفات — Directory Bruteforcing">
            <p>كثير من الـ endpoints الحساسة لا تكون مرتبطة في الواجهة. الـ bruteforcing يكشفها.</p>
            <Terminal lines={[
              { p: "ffuf -u https://target.gov/FUZZ -w /usr/share/seclists/Discovery/Web-Content/raft-large-words.txt -mc 200,301,403 -fs 0" },
              { o: "/admin              [301]\n/.git/config        [200]\n/api/v1/users       [200]\n/backup.zip         [200]\n/phpinfo.php        [200]" },
            ]} />
            <Callout kind="info" title="نصيحة">استخدم SecLists — أكبر مجموعة قوائم مستخدمة في الأمن السيبراني.</Callout>
          </Section>
          <Section title="تعداد الخدمات الشائعة">
            <h3>SMB (Windows)</h3>
...
bloodhound-python -d corp.local -u user -p pass -c All -ns target`}</Code>
          ...
          <Section title="الدفاع ضد المسح">
            <ul>
              <li>قاعدة deny by default في الـ firewall — لا تفتح إلا ما يلزم.</li>
              <li>Rate limiting + SYN cookies ضد الـ SYN scan.</li>
              <li>IDS / Suricata / Snort مع قواعد ET-OPEN لكشف بصمات nmap, nuclei, ffuf.</li>
              <li>Honeypots — منافذ مفتوحة عمداً تسجل كل من يقترب.</li>
              <li>إخفاء البـانرات (banner masking) لإرباك بصمة الإصدار.</li>
            </ul>
